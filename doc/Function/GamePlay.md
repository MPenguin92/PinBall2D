...
| 组件 | 职责 |
|------|------|
| **GameLogicManager**（单例） | 整局入口；维护 `GameState`；`UpdateGame()` 仅在 **Running** 时执行并统一驱动 Player / PinBall / Unit / PlayerRender 的 `Tick`；处理 Unit 触底伤害、玩家死亡判定；**按 `Defines.StepInterval` 节拍广播 `OnStep`**；发送其它 `GameEvents` 生命周期事件 |
...
| 事件 | 触发时机 | 典型订阅方 |
|------|---------|-----------|
| `OnGameStart` | `StartGame()` 初始化完成后 | UIManager（隐藏所有 UI）、UnitCreator（进入运行） |
...
| `OnStep` | Running 下每 `Defines.StepInterval` 秒一次 | 所有活跃 `UnitBase`（启动移动）、UnitCreator（生成新一批） |
...
- **出池**：`SetParent(null)`、`SetActive(true)`；`UnitBase.OnEnable` 自动订阅 `OnStep`，并加入活跃列表参与 Tick。
...
- 管理脚本：`Assets/1_Scripts/Mgr/GameLogicManager.cs`、`Mgr/PoolManager.cs`、`Mgr/UIManager.cs`、`Mgr/GameEvents.cs`、`Mgr/GameEnum.cs`、`Mgr/Defines.cs`、`Mgr/Difficulty.cs`
