...
60,1,3,6,1,1.0,2
120,2,4,9,1,0.85,2
180,7,10,24,1,0.42,4
240,8,11,33,1,0.36,5
300,9,12,45,1,0.32,6
360,10,13,60,1,0.28,8
420,11,14,78,1,0.25,10
480,12,15,99,1,0.22,12
540,13,16,123,1,0.2,14
600,14,17,150,1,0.18,16
660,15,18,180,1,0.16,18
720,16,19,216,1,0.145,20
780,17,20,255,1,0.13,23
840,18,21,300,1,0.115,26
900,19,22,348,1,0.1,30
960,20,23,402,1,0.09,34
1020,21,24,462,1,0.08,38
1080,22,25,528,1,0.07,42
1140,23,26,600,1,0.06,46
1200,24,28,675,1,0.05,50
```

设计解读：

- 前 30 秒使用 `spawn=1`、`hp=3`、`stepInterval=1.2`、`exp=1`，给玩家最低压力并开始积累升级经验。
- 30-60 秒只增加生成上限到 2，并略微加快 Step。
- 60-120 秒开始出现 6 血 Unit，经验升至 2，整体仍保持可通过。
- 120-180 秒进入过渡期，生成数量和血量都增加，但还没有完全压死玩家。
- 180 秒处是第一段明显断崖：`spawnMin=7`、`spawnMax=10`、`unitHp=24`、`stepInterval=0.42`、`exp=4`。由于 `unitAttack=1` 固定，这里通过高密度和快速推进让漏怪数量迅速累积；同时经验提高，促使玩家更频繁拿到升级。
...
相对旧版曲线：`unitHp` 整体 ×3（开局 `1→3`）；球 `InitialSpeed` `12→24`；`stepInterval` 相对血量上调前再整体约 ×1.2（开局 `1.0→1.2`，180s `0.35→0.42`），略放慢刷怪与下移。
